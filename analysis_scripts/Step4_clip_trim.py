...
from config import get_ffmpeg_path, get_trimming_defaults, get_evoked_root, get_processed_evoked_root, get_metadata_path

# Script Authors:    Darian Mohsenin & Google Gemini
# Date: 3/23/2026

# About:
# Batch trimmming of evoked clips across multiple mice
# ====================================================================
# Update Log:
...
# ====================================================================
# FFMPEG AND LOGGING CONFIG
# ====================================================================
# FFmpeg and Logging (configurable)
FFMPEG_PATH = get_ffmpeg_path() or r"C:\Users\dmohsenin\Desktop\ffmpeg\bin\ffmpeg.exe"
LOG_FILENAME = 'batch_trimming_log.txt' # CURRENTLY NOT SAVED!! (SEE BELOW)
VIDEO_EXTENSION = ".avi"
VIDEO_KEYWORD = "evoked"

# Trimming defaults from config
_TRIM_DEF = get_trimming_defaults()

# Setup basic logging configuration
logging.basicConfig(
    # filename=LOG_FILENAME,       COMMENT IN IF SCRIPT SPECIFIC LOGGING IS NEEDED!
    level=logging.INFO,
...
# ====================================================================
# MATCH '.AVI' BEHAVIOR VIDEOS TO '.CSV' CORRECTED FRAME FILES BY UNIQUE_ID
# ====================================================================
...
def find_file_matches(input_dir: str) -> Dict[str, Tuple[str, str]]:
    ...
    print(f"\nFound {len(matches)} video + csv file pairs to process.")
    return matches
...
def main():
    ...
    print("Clean Batch Video Trimming Tool\n" + "="*40)
    ...
    if env_proj and env_expid:
        ...
        try:
            ...
            number_input = env_expid.zfill(3)
            INPUT_BASE_DIR = str(get_evoked_root(project_type))
            OUTPUT_BASE_DIR = str(get_processed_evoked_root(project_type))
            METADATA_PATH = str(get_metadata_path(project_type))
        except Exception as e:
            print(f"ERROR setting up environment: {e}")
            return
    else:
        # --- MANUAL MODE (Interactive) ---
        project_type_input = input("Project: TN (1) or TMJ (2): ").strip()
        if not project_type_input: return
        project_type = int(project_type_input)
        number_input = input("Enter Experiment Number (e.g., 001): ").strip().zfill(3)
    ...
    if not exp_folder_name:
        print(f"Could not find experiment folder for {number_input}")
        return
    ...
    # 3. GET TRIMMING PARAMETERS
    before = _TRIM_DEF.get('before_seconds', 30.0)
    after = _TRIM_DEF.get('after_seconds', 30.0)
    fps = _TRIM_DEF.get('default_fps', 70.0)

    # 4. RUN PROCESSING
    matches = find_file_matches(input_dir)
    if not matches:
        print("\n[ALERT] No matching video and CSV pairs found.")
    else:
        for file_id, (video_path, csv_path) in matches.items():
            process_file_pair(
                file_id, csv_path, video_path, main_output_dir,
                before, after, fps
            )
        
        # Finally, update the global metadata
        update_global_metadata_csv(exp_folder_name, METADATA_PATH)
        print("\nBatch processing complete!")

    if not env_proj: # Only pause if running manually
        input("\nPress Enter to exit...")
